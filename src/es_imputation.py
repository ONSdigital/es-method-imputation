...
def doMovement(input_data, period):
    ...
    merged_df.drop(
        ['Q601_asphalting_sand_x', 'Q602_building_soft_sand_x', 'Q603_concreting_sand_x', 'Q604_bituminous_gravel_x',
         'Q605_concreting_gravel_x',
         'Q606_other_gravel_x', 'Q607_constructional_fill_x', 'Q608_total_x', 'period_x', 'period_y'], axis=1,
        inplace=True)
...
def MovementCalc(previous, current):
    '''
               ...
               '''
    if int(previous) > 0:
        Movement = ((int(current) - int(previous)) / int(previous))
    else:
        Movement = 0
    return Movement
...
def calculateMeans(dataframe1, dataframe2):
    ...
    sums.rename(columns={'movement_Q601_asphalting_sand': 'movement_Q601_asphalting_sand_aggregated',
                         ...
                         'movement_Q607_constructional_fill': 'movement_Q607_constructional_fill_aggregated'},
                inplace=True)
    ...
    dataframe2.drop(
        ['movement_Q601_asphalting_sand', 'movement_Q602_building_soft_sand', 'movement_Q603_concreting_sand',
...
def remAtypicals(row):
    ...
    def docalculation(row, qval, x):
        ...
        row['atypical_' + str(qval)] = abs(row['movement_' + str(qval)] - row['mean_60' + str(x)]) - 2 * row[
            'iqr_60' + str(x)]
...
def recalc_means(working_data):
    ...
    working_data = working_data[['region', 'strata', 'land_or_marine', 'movement_Q601_asphalting_sand',
                                 'movement_Q602_building_soft_sand', 'movement_Q603_concreting_sand',
    ...
    counts = working_data.groupby(['region', 'strata'])[
        ['movement_Q601_asphalting_sand', 'movement_Q602_building_soft_sand', 'movement_Q603_concreting_sand',
         'movement_Q604_bituminous_gravel', 'movement_Q605_concreting_gravel', 'movement_Q606_other_gravel',
...
def applyFactors(row):
    ...
    row['Q608_total'] = sum((row['Q601_asphalting_sand'], row['Q602_building_soft_sand'], row['Q603_concreting_sand'],
                             row['Q604_bituminous_gravel'], row['Q605_concreting_gravel'], row['Q606_other_gravel'],
                             row['Q607_constructional_fill']))
    return row
...
def save_imputed_values(imputeddataset, disagg, period):
    ...
    imputeddataset.drop(
        ['Previous_Q601_asphalting_sand', 'Previous_Q602_building_soft_sand', 'Previous_Q603_concreting_sand',
         'Previous_Q604_bituminous_gravel', 'Previous_Q605_concreting_gravel', 'Previous_Q606_other_gravel',
         'Previous_Q607_constructional_fill', 'Previous_Q608_total'
    ...
    disaggNotCurrent = disagg[
        ~((disagg.period == period) & (disagg['responder_id'].isin(imputeddataset['responder_id'])))]
